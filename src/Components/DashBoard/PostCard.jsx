...
async function deleteHandler () {
  const response = await dispatch(deletePost({id: post._id, authorId : post.author}));
      ...
      <div className="card mb-3 bg-base-100 shadow-xl lg:card-side">
        ...
        <div className="card-body lg:w-9/12">
          <div className="mt-1 flex items-center gap-2 font-semibold">
            <div>
              <span className={`${isActive ? "text-green-500" : "text-red-800"}`}>
                {isActive ? "Active" : "Inactive"}
              </span>
            </div>
            <div className="ms-auto flex gap-2 items-center justify-center">
            <input type="checkbox" className="toggle toggle-success toggle-sm" checked={isActive}  onChange={postStatus}/>

              <button  onClick={()=>document.getElementById('my-delete-modal').showModal()}><FaTrash className="w-4 h-4 text-red-600"/></button>
                  <dialog id="my-delete-modal" className="modal">
                    <div className="modal-box">
                      <h3 className="font-bold text-lg text-indigo-700">Confirm Delete</h3>
                      <p className="py-4 text-black">Are you sure you want to delete this post ?</p>
                      <div className="modal-action">
                        
                        <form method="dialog">
                        <button className="btn me-2" onClick={deleteHandler}>Confirm</button>
              ...
              <span className="flex gap-1 items-center text-primary"><FaThumbsUp className="w-4 h-4"/> {post.likes}</span>
            </div>
          </div>
          <h2
